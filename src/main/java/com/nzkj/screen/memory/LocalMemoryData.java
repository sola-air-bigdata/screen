...
import com.nzkj.screen.Repo.GunRepository;
...
//	}
	...
	public List<GunMonitorDto> getGunBySeller(Long sellerId) {
		...
		if(CollectionUtil.isEmpty(stations)) {
			stations = stationRepository.findIdBySeller(sellerId);
			if(CollectionUtil.isEmpty(stations)){
				return Collections.emptyList();
			}else{
				loadData(sellerId);
		...
		for(Long stationId: stations) {
			...
			if(CollectionUtil.isNotEmpty(stationPiles)) {
				...
				stationPiles = pileRepository.findPileByStationId(stationId);
				if(CollectionUtil.isNotEmpty(stationPiles)){
	...
	private void loadData(Long seller){
		...
		stations = stationRepository.findIdBySeller(seller);
		...
		for(Long stationId: stations) {
			...
			if(CollectionUtil.isNotEmpty(stationPiles)){
				...
				for (Long pileId : stationPiles){
					...
					guns = gunRepository.findGunNoByPileId(pileId);
					for(Long gunNo : guns){
