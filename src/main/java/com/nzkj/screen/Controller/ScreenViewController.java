...
import com.nzkj.screen.Repo.AllDayDataRepository;
import com.nzkj.screen.Repo.AllMonthDataRepository;
import com.nzkj.screen.Repo.StationRepository;
import com.nzkj.screen.Repo.TotalOperateDataRepository;
...
public class ScreenViewController {
    ...
    public Map<String, Object> getTotalNum() {
        ...
        if (totalMoney == null || userCount == null || serviceCount == null){
            ...
            Object data = totalOperateDataRepository.queryTotalNum(sellerId);
            Object[] obj = (Object[]) data;
            //存入redis
            redisTemplate.opsForValue().set(totalMoneyCountKey,obj[0],expireTime, TimeUnit.SECONDS);
            redisTemplate.opsForValue().set(totalServiceUserCountKey,obj[1],expireTime,TimeUnit.SECONDS);
            redisTemplate.opsForValue().set(totalServiceCountKey,obj[2],expireTime,TimeUnit.SECONDS);
            //数据放入返回map
            //累计总收入totalMoney元
            map.put("totalMoney", obj[0].toString());
            //累计服务用户数totalServiceMemberNum
            map.put("totalServiceMemberNum", obj[1]);
            //累计服务次数serviceTotalCount
            map.put("serviceTotalCount",obj[2]);
        }else {
            //直接从redis取
            //累计总收入totalMoney元
...
//            map.put("serviceTotalCount",redisTemplate.opsForValue().get(totalServiceCountKey));
    ...
    public Map<String, Object> getAllPowerAndCarCharging(){
        ...
        if (all == null){
        BigInteger allPower = totalOperateDataRepository.getCapacitySum(sellerId);
    ...
    public Map<String, Object> historyTotalCharging(){
        ...
        try {
            ...
            if(sumPower == 0 || temPower == 0){
                //缓存中未能获取到  从数据库获取
                sumPower = ValUtil.toLong(totalOperateDataRepository.getSumPowerCount(sellerId));
                temPower = ValUtil.toLong(totalOperateDataRepository.getTemPowerCount(sellerId));
                //获取到的数据再缓存到redis
    ...
    public Map<String, Object> getStationServiceRanking(){
        ...
        try {
            List<Object> objList = totalOperateDataRepository.getStationServiceList(sellerId, TimeUtils.format(new Date(), "yyyy-MM"));
            if( !CollectionUtils.isEmpty(objList) ) {
                for (Object object : objList) {
                    Object[] obj = (Object[]) object;
                    Map<String, String> mapData = new HashMap<>();
                    mapData.put("stationName", ValUtil.toString(obj[0]));
                    mapData.put("chargeCount", ValUtil.toString(obj[2], "0"));
                    Object allCount = ValUtil.toLong(obj[1]);
                    Object gunCount = ValUtil.toLong(obj[3]);
    ...
    public Map<String, Object> serviceOperateData(){
        ...
        try {
            ...
            if(totalServiceCount == null || teamServiceCount == null ||
                ...
                //redis缓存中未取到 从数据库取
                res = totalOperateDataRepository.getServiceRunData(sellerId);
                Object[] obj = (Object[]) res;
                // 将数据库中获取的数据更新到redis中
                // 1 累计服务次数
                redisTemplate.opsForValue().set(totalServiceCountKey,obj[0],expireTime,TimeUnit.SECONDS);
                // 2 车队累计服务次数
                redisTemplate.opsForValue().set(teamServiceCountKey,obj[1],expireTime,TimeUnit.SECONDS);
                // 3 其他服务次数
                redisTemplate.opsForValue().set(personalServiceCountKey,obj[2],expireTime,TimeUnit.SECONDS);
                // 4 累计服务总时长
                redisTemplate.opsForValue().set(totalServiceTimeKey,obj[3],expireTime,TimeUnit.SECONDS);
                // 5 车队累计服务时长
                redisTemplate.opsForValue().set(teamServiceTimeKey,obj[4],expireTime,TimeUnit.SECONDS);
                // 6 其他累计服务时长
                redisTemplate.opsForValue().set(personalServiceTimeKey,obj[5],expireTime,TimeUnit.SECONDS);

                dto.setAllChargeCount(ValUtil.toLong(obj[0], 0L));
                dto.setTeamChargeCount(ValUtil.toLong(obj[1], 0L));
                dto.setOtherChargeCount(ValUtil.toLong(obj[2], 0L));
                dto.setAllChargeTime(ValUtil.toLong(obj[3], 0L));
                dto.setTeamChargeTime(ValUtil.toLong(obj[4], 0L));
                dto.setOtherChargeTime(ValUtil.toLong(obj[5], 0L));
    ...
    public Map<String, Object> incomeWayData(){
        ...
        try {
            ...
            if(totalWechat == null || totalApp == null || totalCard == null || totalVin == null) {
                ...
                obj = totalOperateDataRepository.getTotalBalanceData(sellerId);
                ...
                map.put("wx", ValUtil.toLong(rs[0],0L));
                map.put("app", ValUtil.toLong(rs[1],0L));
                map.put("card", ValUtil.toLong(rs[2],0L));
                map.put("vin", ValUtil.toLong(rs[3],0L));
    ...
    public Map incomeOperate(){
        ...
        try {
            ...
            if (totalServiceUserCount == null || teamActualBalance == null || personalActualBalance == null
                ...
                object = totalOperateDataRepository.getServiceUserCount(sellerId);
                Object[] obj = (Object[]) object;

                redisTemplate.opsForValue().set(totalServiceUserCountKey,obj[0],expireTime,TimeUnit.SECONDS);
                redisTemplate.opsForValue().set(teamActualBalanceKey,obj[0],expireTime,TimeUnit.SECONDS);
                redisTemplate.opsForValue().set(personalActualBalanceKey,obj[0],expireTime,TimeUnit.SECONDS);
                redisTemplate.opsForValue().set(totalPowerBalanceKey,obj[0],expireTime,TimeUnit.SECONDS);
                redisTemplate.opsForValue().set(totalserviceBalanceKey,obj[0],expireTime,TimeUnit.SECONDS);

                dto.setTotalIncome(ValUtil.toLong(obj[0], 0L));

                powerBalance = ValUtil.toLong(obj[1], 0L);
                otherBalance = ValUtil.toLong(obj[2], 0L);
                teamBalan = ValUtil.toLong(obj[3], 0L);
                personageBalance = ValUtil.toLong(obj[4], 0L);
    ...
    public Map<String, Object> stationAllData(){
        ...
        try {
            ...
            if(redisTemplate.opsForValue().get(totalStationKey) == null || redisTemplate.opsForValue().get(foreignKey) == null || redisTemplate.opsForValue().get(internalKey) == null || redisTemplate.opsForValue().get(totalServiceTimeKey) == null
                ...
                resDate = totalOperateDataRepository.getHomeStationData(sellerId);
            }
            //充电站总数量
            if(redisTemplate.opsForValue().get(totalStationKey) == null  && resDate != null){
                Object[] obj = (Object[]) resDate;
                dto.setStatioTotal(ValUtil.toInteger(obj[0], 0));
                redisTemplate.opsForValue().set(totalStationKey, obj[0],expireTime,TimeUnit.SECONDS);
            }else{
            ...
            if(redisTemplate.opsForValue().get(foreignKey) == null && resDate != null){
                Object[] obj = (Object[]) resDate;
                dto.setOutsideCount(ValUtil.toInteger(obj[1], 0));
                redisTemplate.opsForValue().set(foreignKey, obj[1],expireTime,TimeUnit.SECONDS);
            }else{
            ...
            if(redisTemplate.opsForValue().get(internalKey) == null && resDate != null){
                Object[] obj = (Object[]) resDate;
                dto.setInsideCount(ValUtil.toInteger(obj[2], 0));
                redisTemplate.opsForValue().set(internalKey, obj[2],expireTime,TimeUnit.SECONDS);
            }else{
            ...
            if(redisTemplate.opsForValue().get(totalServiceTimeKey) == null && resDate != null ){
                Object[] obj = (Object[]) resDate;
                dto.setInConstructionCount(ValUtil.toInteger(obj[3], 0));
                redisTemplate.opsForValue().set(totalServiceTimeKey, obj[3],expireTime,TimeUnit.SECONDS);
            }else{
            ...
            if(redisTemplate.opsForValue().get(bebuiltStationKey) == null && resDate != null ){
                Object[] obj = (Object[]) resDate;
                dto.setToBeBulitCount(ValUtil.toInteger(obj[4], 0));
                redisTemplate.opsForValue().set(bebuiltStationKey, obj[4],expireTime,TimeUnit.SECONDS);
            }else{
            ...
            if(redisTemplate.opsForValue().get(chargingPileKey) == null && resDate != null){
                Object[] obj = (Object[]) resDate;
                dto.setPileCount(ValUtil.toInteger(obj[5], 0));
                redisTemplate.opsForValue().set(chargingPileKey, obj[5],expireTime,TimeUnit.SECONDS);
            }else{
            ...
            if(redisTemplate.opsForValue().get(chargingGunKey) == null && resDate != null ){
                Object[] obj = (Object[]) resDate;
                dto.setGunCount(ValUtil.toInteger(obj[6], 0));
                redisTemplate.opsForValue().set(chargingGunKey, obj[6],expireTime,TimeUnit.SECONDS);
            }else{
    ...
    public Map<String, Object> doGetRealPower(){
        ...
        if(allPower == 0){
            allPower = totalOperateDataRepository.getCapacitySum(sellerId).longValue();
        }
...
//        }
    ...
    private JSONArray getStationInfo(){
        ...
        if(redisTemplate.opsForValue().get(stationInfoUnderSellerId) != null){
            ...
            List<Station> stations = stationRepository.findBySellIdAndDeleteFlag(sellerId,false);
        ...
        for (String moth: timeList) {
            ...
            if (redisTemplate.opsForValue().get(chargeBalanceKey) == null || redisTemplate.opsForValue().get(otheKey) == null || redisTemplate.opsForValue().get(otheKey) == null || redisTemplate.opsForValue().get(personaOtherKey) == null) {
                if(condition.getMemberTypeEnum() != null &&"1".equals(condition.getMemberTypeEnum().getCode().shortValue())){
                    res = allMonthDataRepository.getPersonRevenueTrendList(sellerId,condition.getStartTime());

                }else {
                    res = allMonthDataRepository.getRevenueTrendList(sellerId,condition.getStartTime());
                }
            }
            ...
            if(condition.getMemberTypeEnum() != null && "1".equals(condition.getMemberTypeEnum().getCode().shortValue())){
                if (redisTemplate.opsForValue().get(personalBlanceKey) == null) {
                    ...
                    if (res != null) {
                        //2.1.1取出的数据存进redis
                        Object[] obj = (Object[]) res;
                        redisTemplate.opsForValue().set(personalBlanceKey, obj[0],expireTime,TimeUnit.SECONDS);
                        Long chargeBalance = ValUtil.toLong(obj[0], 0L);
                ...
                if (redisTemplate.opsForValue().get(personaOtherKey) == null) {
                    ...
                    if (res != null) {
                        //2.2.1取出的数据存进redis
                        Object[] ob = (Object[]) res;
                        redisTemplate.opsForValue().set(personaOtherKey, ob[1],expireTime,TimeUnit.SECONDS);
                        Long personageOtherRevenue = ValUtil.toLong(ob[1], 0L);
                ...
                if (redisTemplate.opsForValue().get(chargeBalanceKey) == null) {
                    ...
                    if (res != null) {
                        //3.1.1取出的数据存进redis
                        Object[] obj = (Object[]) res;
                        redisTemplate.opsForValue().set(chargeBalanceKey, obj[0],expireTime,TimeUnit.SECONDS);
                        Long chargeBalance = ValUtil.toLong(obj[0], 0L);
                        chargeBalanceList.add(chargeBalance);
                    } else {
                ...
                if (redisTemplate.opsForValue().get(personaOtherKey) == null) {
                    ...
                    if (res != null) {
                        ...
                        Object[] ob = (Object[]) res;
                        redisTemplate.opsForValue().set(personaOtherKey, ob[1],expireTime,TimeUnit.SECONDS);
                        Long personageCount = ValUtil.toLong(ob[1], 0L);
    ...
    private Map<String, Object> getMemberChargeMap(SelectCondition condition) {
        ...
        for (String date: timeList){
            ...
            if( redisTemplate.opsForValue().get(temkey) == null ){
                res =  allDayDataRepository.getUserActivityList(sellerId,condition.getStartTime());
                //1,2如果数据库有数据从数据库中取
                if( res != null ){
                    //1.2.1取出的数据存进redis
                    Object[] obj = (Object[]) res;
                    redisTemplate.opsForValue().set(temkey, obj[0],expireTime,TimeUnit.SECONDS);
                    Integer teamCount = ValUtil.toInteger(obj[0], 0);
            ...
            if( redisTemplate.opsForValue().get(personalkey) == null ){
                ...
                if( res != null ){
                    //1.2.1取出的数据存进redis
                    Object[] ob = (Object[]) res;
                    redisTemplate.opsForValue().set(personalkey, ob[1],expireTime,TimeUnit.SECONDS);
                    Integer personageCount = ValUtil.toInteger(ob[1], 0);
        ...
        try {
            ...
            if(redisTemplate.opsForValue().get(dayMemberCountKey) == null || redisTemplate.opsForValue().get(dayPersonalCountKey) == null  || redisTemplate.opsForValue().get(mothMemberCountKey) == null || redisTemplate.opsForValue().get(mothPersonalCountKey) == null){
                if(condition.getTimeSharingEnum().getCoetx() == 3){
                    obj = allDayDataRepository.getPlatformMemberCount(sellerId, TimeUtils.format(condition.getStartDate(),"yyyy-MM-dd"));
                }else {
                    //(condition.getTimeSharingEnum().getCoetx() == 2)
                    obj = allMonthDataRepository.getPlatformMemberCount(sellerId, TimeUtils.format(condition.getStartDate(),"yyyy-MM"));
                }
            }
            //1如果传的日期不为空，类型为 2（月）
            if( condition.getStartDate()!= null  && condition.getTimeSharingEnum() != null && condition.getTimeSharingEnum().getCoetx() == 2 ){
                ...
                if(redisTemplate.opsForValue().get(mothMemberCountKey) == null ){
                    if(obj != null) {
                        ...
                        dayMemberCount = ValUtil.toInteger(res[0], 0);
                ...
                if(redisTemplate.opsForValue().get(mothPersonalCountKey) == null ){
                    if(obj != null) {
                        Object[] res = (Object[]) obj;
                        dayPersonalCount = ValUtil.toInteger(res[1], 0);
    ...
    private Map<String, Object> getMemberAddMap(SelectCondition condition) {
        ...
        for (String date: timeList){
            ...
            if( redisTemplate.opsForValue().get(temkey) == null ){
                Object[] ob = null;
                if(condition.getTimeSharingEnum().getCoetx() == 3){
                    ob = (Object[])allDayDataRepository.getPlatformMemberCount(sellerId,date);
                }else {
                    //(condition.getTimeSharingEnum().getCoetx() == 2)
                    ob = (Object[])allMonthDataRepository.getPlatformMemberCount(sellerId,date);
                }
                //1,1如果数据库有数据从数据库中取
                if( ob!= null ){
                    //1.1.1、取出的数据存进redis
                    redisTemplate.opsForValue().set(temkey, ob[0],expireTime,TimeUnit.SECONDS);
                    Integer teamCount = ValUtil.toInteger(ob[0], 0);
                    teamList.add(teamCount);
                }
                else {
    ...
    private ServiceMemberDto getServiceMemberDto(SelectCondition condition) {
        ...
        try {
            ...
            if(redisTemplate.opsForValue().get(appUserKey) == null || redisTemplate.opsForValue().get(wechatUserKey) == null || redisTemplate.opsForValue().get(personanlKey) == null || redisTemplate.opsForValue().get(teamKey) == null){
                object = allDayDataRepository.getMemberInfo(sellerId, TimeUtils.format(condition.getStartDate(),"yyyy-MM-dd"));
            }
            //app
            if(redisTemplate.opsForValue().get(appUserKey) == null && object != null ){
                Object[] obj = (Object[]) object;
                dto.setAppCount(ValUtil.toInteger(obj[2], 0));
            }else{
            ...
            if(redisTemplate.opsForValue().get(wechatUserKey) == null && object != null ){
                Object[] obj = (Object[]) object;
                dto.setWxCount(ValUtil.toInteger(obj[3], 0));
            }else{
            ...
            if(redisTemplate.opsForValue().get(teamKey) == null && object != null ){
                Object[] obj = (Object[]) object;
                dto.setTeamCount(ValUtil.toInteger(obj[0], 0));
            }else{
            ...
            if(redisTemplate.opsForValue().get(personanlKey) == null && object != null ){
                Object[] obj = (Object[]) object;
                dto.setPersonageCount(ValUtil.toInteger(obj[1], 0));
            }else{
    ...
    private Map<String, Object> getMemberAddByWeek(SelectCondition condition) {
        ...
        for (String dateTime: timeList) {
            ...
            if (redisTemplate.opsForValue().get(dayGrowthTeamUserKey) == null || redisTemplate.opsForValue().get(dayGrowthPersonalUserKey) == null ) {
                res = allDayDataRepository.getMemberAddRecordList(sellerId,dateTime);
            }
            ...
            if (redisTemplate.opsForValue().get(dayGrowthTeamUserKey) == null) {
                ...
                if (res != null) {
                    //3.1.1取出的数据存进redis
                    Object[] obj = (Object[]) res;
                    redisTemplate.opsForValue().set(dayGrowthTeamUserKey, obj[0],expireTime,TimeUnit.SECONDS);
                    Integer teamCount = ValUtil.toInteger(obj[0]);
            ...
            if (redisTemplate.opsForValue().get(dayGrowthPersonalUserKey) == null) {
                ...
                if (res != null) {
                    //4.1.1 取出的数据存进redis
                    Object[] obj = (Object[]) res;
                    redisTemplate.opsForValue().set(dayGrowthPersonalUserKey, obj[1],expireTime,TimeUnit.SECONDS);
                    Integer personal = ValUtil.toInteger(obj[1]);
